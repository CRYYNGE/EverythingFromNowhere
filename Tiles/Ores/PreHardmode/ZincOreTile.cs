...
namespace EverythingFromNowhere.Tiles.Ores.PreHardmode
{
    public class ZincOreTile : ModTile
    {
        public override void SetStaticDefaults()
        {
            ...
            DustType = 84;
            ItemDrop = ModContent.ItemType<Items.Materials.Ores.ZincOreItem>();
    ...
    public class ZincGen : ModSystem
    {
        public override void ModifyWorldGenTasks(List<GenPass> tasks, ref float totalWeight)
        {
            ...
            if (ShiniesIndex != -1)
            {
                tasks.Insert(ShiniesIndex + 1, new ZincOrePass("Zinc", 237.4298f));
            }
        }

        public class ZincOrePass : GenPass
        {
            public ZincOrePass(string name, float loadWeight) : base(name, loadWeight)
            {
            }

            protected override void ApplyPass(GenerationProgress progress, GameConfiguration configuration)
            {
                progress.Message = "Placing Zinc...";
